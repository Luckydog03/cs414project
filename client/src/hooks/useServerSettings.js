...
export function useServerSettings(showMessage) {
    ...
    async function sendConfigRequest() {
        const configResponse = await sendAPIRequest(
            { requestType: "config" },
            serverUrl,
        );
        if (configResponse) {
            ...
            showMessage(
                `Config request to ${serverUrl} failed. Check the log for more details.`,
                "error",
            );
        }
    }
    ...
    return [
        { serverUrl: serverUrl, serverConfig: serverConfig },
